...
import com.sevenstar.util.DateTimeUtil;
import com.sevenstar.util.FileUtil;
 ...
 * Created :  java
 * Date    :  3/26/15
 * Note聽: Some codes come from internet
 */
public class MteSenseCore {
    ...
    private Screen sikuliDriver = null;
    ...
    private MSAssert msAssert= null;
    ...
    public MteSenseCore(Boolean ndriver){
    	this.driver = null;
    }
...
//                logger.error("isElementPresentDisplayed error :", e);
    ...
    /**
     * wait for the specified element exist with timeout setting.
     *
     * @param locator the element locator on the page
     * @param timeout  wait Seconds for locator
     */
    public boolean isElementExist(By locator,long timeout){
    	pause(timeout*1000);
    	return driver.findElements(locator).size()!=0;
    }
    
    public boolean isElementExist(By locator){
    	return isElementExist(locator,0);
    }
...
//            logger.error("pause error:", e);
  ...
  //          logger.error("Save screencapture failed锛�", e);
        }
    }
 ...
 //       logger.debug("Attribute is [ " + value + " ]");
    ...
    /**
     * rewrite the sendKeys method, adding user defined log銆�
     *
     * @param element the webelement you want to operate
     */
     ...
     * 
     * @param by_O the element you Drag
     * @param by_T the element you want to Drop
     */

    public void DragAndDrop(By by_O,By by_T){
    	new Actions(driver).dragAndDrop(driver.findElement(by_O), driver.findElement(by_T)).perform();
    }
    
    /**
     * Focus on the element
     * 
     * @param by   The element you want to focus on.
     */
...
//        logger.debug("current window handle is:" + handle);
 ...
 //       logger.debug("element's TagName is: " + tagName);
  ...
  //      logger.debug("element [ " + by.toString() + " ]'s TagName is: " + tagName);
        return tagName;
    }
 ...
 //       logger.debug("element [ " + by.toString() + " ]'s text is: " + text);
  ...
  //      logger.debug("element's css [" + propertyName + "] value is:"+ cssValue);
        return cssValue;
    }
   ...
   //     logger.debug("element [ " + by.toString() + " ]'s css[" + propertyName     + "] value is: " + cssValue);
        return cssValue;
    }
    ...
    private Set<String> clearHandleCache(Set<String> windowHandles) {
        ...
        for (String handle : windowHandles) {
            try {
                ...
                errors.add(handle);
  //              logger.debug("window handle " + handle + " does not exist acturely!");
            }
        }
 ...
 //               logger.error(e);
  ...
  //          logger.debug("no opened windows!");
    ...
    public void closeWindow(String windowTitle, int index) {
        ...
        try {
            ...
            driver.close();
  //          logger.debug("window [ " + windowTitle + " ] closed by index ["  + index + "]");
            isSucceed = true;
        } catch (Exception e) {
   //         logger.error(e);
        }
  ...
  //              logger.error(e);
   ...
   //         logger.debug("input text [ " + text + " ] to frame by index [ " + index + " ]");
            isSucceed = true;
        } catch (Exception e) {
  //          logger.error(e);
        }
   ...
   //         logger.error(e);
    ...
    protected void editFrameText(By by, String text) {
        ...
        try {
            driver.switchTo().frame(driver.findElement(by));
            driver.switchTo().activeElement().sendKeys(text);
   //         logger.debug("input text [ " + text + " ] to frame [ "+ by.toString() + " ]");
            isSucceed = true;
        } catch (Exception e) {
    //       logger.error(e);
        }
...
//                logger.debug("item selected by item value [ " + itemValue+ " ] on [ " + by.toString() + " ]");
                isSucceed = true;
            }
        } catch (Exception e) {
 //           logger.error(e);
 ...
 //               logger.debug("item selected by visible text [ " + text+ " ] on [ " + by.toString() + " ]");
                isSucceed = true;
            }
        } catch (Exception e) {
   //         logger.error(e);
        ...
        try {
            if (isElementPresent(by, timeout)) {
                ...
                if ((onOrOff.toLowerCase().contains("on") && !checkBox
                        ...
                        .isSelected())) {
                    checkBox.click();
 //                   logger.debug("the checkbox [ " + by.toString()+ " ] is set to [ " + onOrOff.toUpperCase() + " ]");
                } else {
                    if ((onOrOff.toLowerCase().contains("on") && checkBox
                            .isSelected())) {
 //                       logger.debug("the checkbox [ " + by.toString()+ " ] is already set to [ " + onOrOff.toUpperCase()+ " ]");
                    }
        ...
        try {
            if (isElementPresent(by, timeout)) {
                ...
                if ((onOrOff.toLowerCase().contains("on") && !radioGroup
                        ...
                        .isSelected())) {
                    radioGroup.click();
 //                   logger.debug("the radioGroup [ " + by.toString()+ " ] is set to [ " + onOrOff.toUpperCase() + " ]");
                } else {
                    if ((onOrOff.toLowerCase().contains("on") && radioGroup
                            .isSelected())) {
//                        logger.debug("the radioGroup [ " + by.toString()+ " ] is already set to [ " + onOrOff.toUpperCase()+ " ]");
                    }
 ...
 //           logger.error(e);
  ...
  //      logger.debug("execute js [ " + js + " ]");
   ...
   //     logger.debug("execute js [ : " + js + " ],arguments is : "+ args.toString());
   ...
   //     logger.debug("execute js [ : " + js + " ],arguments is : "+ args.toString());
    ...
    public void runWindowsApplication(String appPath){
	...
	/**
	 * retrieveService data from svcURL
	 * 
	 * @param svcURL  url of the service
	 */
	...
	public void postLogin(String svcURL,String user,String pwd) {
		String js_GetResponsData = 
				"var request = new XMLHttpRequest();"+
				"request.open('POST', '"+svcURL+"/pkmslogin.form',true);"+
				"request.setRequestHeader('Content-type','application/x-www-form-urlencoded'); " +
				"request.send('ibm-submit=Sign in&login-form-type=pwd&username="+user+"&password="+pwd+"');";

		pause(2000);
		jsExecutor(js_GetResponsData);
	}
	...
	public void sikuli_Exist(String savedImg,double score,String description){
		pause(1000);
		String browserType = props.get("mte.test.platform");
		String saved_img = props.get("mte.sikuli.savedPath")+"/"+browserType+"_"+savedImg+".png";
		
		Match mt = sikuliDriver.exists(saved_img);
		
		String capturedImg = mt.saveScreenCapture(props.get("mte.sikuli.capturePath"), browserType+"_"+savedImg+".png");
		String savedImgurl = test.addScreenCapture("../"+saved_img);
		String captureImgurl = test.addScreenCapture(capturedImg);
		Double realScore = mt.getScore()*100;
		if(realScore>=score){
			test.log(LogStatus.PASS, "Image", description+"<br>Expected Image: " + savedImgurl +"Actual Image (score:"+realScore+"): "+captureImgurl);
		}else{
			test.log(LogStatus.FAIL, "Image", description+"<br>Expected Image: " + savedImgurl +"Actual Image (score:"+realScore+"): "+captureImgurl);
		}
	}
	
	public void sikuli_Exist(String savedImg,double score){
		sikuli_Exist(savedImg,score,"");
	}
	
	public void sikuli_Exist(String savedImg){
	    sikuli_Exist(savedImg,98);
	}
	/**
	 * Sikuli Click
	 * 
	...
	public void sikuli_Click(String savedImg,double score){
		pause(1000);
		String browserType = props.get("mte.test.platform");
		String saved_img = props.get("mte.sikuli.savedPath")+"/"+browserType+"_"+savedImg+".png";
		...
		String capturedImg = mt.saveScreenCapture(props.get("mte.sikuli.capturePath"), browserType+"_"+saved_img+".png");
		String savedImgurl = test.addScreenCapture("../"+saved_img);
		String captureImgurl = test.addScreenCapture(capturedImg);
		Double realScore = mt.getScore()*100;
		if(realScore>=score){
			try{
				sikuliDriver.click(saved_img);
			}catch(Exception e){
				e.printStackTrace();
			}
		}else{
			test.log(LogStatus.FAIL, "Image", "Click Failed (No found Image):"+"<br>Expected Image: " + savedImgurl +"Actual Image (score:"+realScore+"): "+captureImgurl);
		}
	}
	
	public void sikuli_Click(String saveImg){
		sikuli_Click(saveImg,98);
	}
	
	public void sikuli_hover(String saveImg) throws FindFailed{
		pause(1000);
		sikuliDriver.mouseMove(getsave_Img(saveImg));
	}
	
	public String getsave_Img(String saveImg){
		String browserType = props.get("mte.test.platform");
		String saved_img = props.get("mte.sikuli.savedPath")+"/"+browserType+"_"+saveImg+".png";
		if(FileUtil.exists(saved_img)){
			return saved_img;
		}else{
			saved_img = props.get("mte.sikuli.savedPath")+"/"+"ALL_"+saveImg+".png";
		}
		return saved_img;
	}
